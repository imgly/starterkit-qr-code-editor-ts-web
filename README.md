...
Create stunning graphics with QR code generation. Generate and customize QR codes directly within your designs using the QR Code Generator panel. Built with [CE.SDK](https://img.ly/creative-sdk) by [IMG.LY](https://img.ly), runs entirely in the browser with no server dependencies.

<p>
  <a href="https://img.ly/docs/cesdk/js/plugins/qr-code/">Documentation</a> |
...
│   └── imgly/
│       ├── index.ts          # Editor initialization with QR code plugin
│       ├── plugins/
│       │   └── qr-code.ts    # QR code plugin setup (dock + canvas menu)
│       └── config/
